...
export default function PushResultsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* 시각 순서: 세그먼트별 전환(order-1) → 푸시 전환 결과 조회(order-2) → 커버리지(order-3) */}
      ...
      <section className="order-3 overflow-hidden rounded-xl border border-slate-200 bg-white">
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-4 py-2.5">
          <div>
            ...
            <p className="text-xs text-slate-400">
              각 푸시를 세그먼트별로 분해 — <b>push 번호 클릭 시 S1 아래로 펼침</b>. 발송·도달·오픈자 수 +
              유효시청률·거래전환율(오픈자 기준). 괄호는 위쪽 세그먼트까지 <b>누적 커버리지</b>(발송·도달·오픈·거래 모두 마지막 행 100%) — 거래는 상위 세그먼트가 전체 구매자의 몇 %를 커버하는지. 발송/도달은 [집계 갱신] 시점 기준.{' '}
        ...
        {!seg ? (
          <p className="px-4 py-6 text-sm text-slate-500">
            세그먼트 집계가 없습니다 — 위 <b>세그먼트별 전환</b>에서 <b>[집계 실행]</b>을 먼저 누르세요.
          </p>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-slate-500">
              <tr>
                ...
                <th className="px-3 py-2 text-right font-medium">유효시청률</th>
                <th className="px-3 py-2 text-right font-medium">거래전환율</th>
              </tr>
            </thead>
            <tbody>
              {coveragePushes.map((p) => {
                ...
                return visibleSegs.map((c, i) => {
                  ...
                  >
                    ...
                    <td className={`px-3 py-1 text-right text-xs tabular-nums${segBorder}`}>{c.view_rate_pct}%</td>
                    <td className={`px-3 py-1 text-right text-xs tabular-nums${segBorder}`}>
                      <span className="font-semibold">{c.purchase_rate_pct}%</span>
                      <span className="ml-1 text-[10px] text-slate-400">{cum('n_purchase')}</span>
                    </td>
                  </tr>
                  )
                })
      ...
      <section className="order-2 rounded-xl border border-slate-200 bg-white p-4">
        <div className="mb-3">
          <h3 className="text-sm font-semibold text-slate-600">푸시 전환 결과 조회 · 커버리지 추가</h3>
          <p className="text-xs leading-relaxed text-slate-400">
            <b>push_seq · 판매자이름 · 카테고리</b> 3개만 입력하면 발송시각 전후 방송을 자동으로 찾아 오픈→유효시청→구매를 미리 보여주고,{' '}
            <b>[커버리지에 추가]</b> 시 위 '세그먼트별 전환·커버리지'에 들어갑니다(이후 <b>[집계 갱신]</b> 필요).
            <br />
      ...
      <section className="order-1 rounded-xl border border-slate-200 bg-white p-4">
        <div className="mb-3 flex items-end justify-between">
          <div>
            <h3 className="text-sm font-semibold text-slate-600">
              세그먼트별 전환{seg ? ` (${seg.period_start.slice(5)}~${seg.period_end.slice(5)} 방송)` : ''}
            </h3>
            <p className="text-xs text-slate-400">
        ...
        {segLoaded && !seg && !segRunning && (
          <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center text-slate-500">
            아직 집계가 없습니다. <b>[집계 실행]</b>을 눌러 세그먼트별 전환을 계산하세요.
          </div>
        )}
