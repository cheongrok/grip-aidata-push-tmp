...
유저를 행동 기반 k-prototypes 세그먼트(10개)로 나누고, 세그먼트별 과거 반응을 근거로
새 푸시를 어느 세그먼트에 보낼지 배분한다. 같은 클릭 수를 더 적은 발송으로 달성하는 것이 목표.

- **백엔드** FastAPI (`backend/`, :8010) — Snowflake 집계 + 세그먼트 스냅샷/분배 API
- **프론트** React 19 + Vite 7 + TS + Tailwind (`frontend/`, :3001)
- **공유 코어** `core/` — 세그먼트 배정(클러스터링)·분배 로직 (백엔드와 분석 노트북이 공유)
...
| 탭 | 하는 일 | 주요 데이터 |
|---|---|---|
| **푸시 현황** | 발송된 푸시의 도달·오픈·전환을 본다. ① 푸시별 오픈율 표/추이 ② **세그먼트별 전환** ③ **메시지 세그먼트별 커버리지** + 오픈 후 방송 자동 매핑(판매자명·push_seq로 방송 발견) | `data_anal.push_result`, 수기 매핑 `cache/push_content_id_map.json` |
| **유저 세그먼트** | [최신화] → 발송가능 전체 모수를 현재 시점 피처로 10개 세그먼트에 배정. 세그먼트별 규모·5월 오픈율·발송 가이드를 한 표로 | `cache/latest_cluster.csv` (회당 Snowflake 약 3~15분) |
...
core/                            백엔드와 분석 노트북이 공유하는 순수 로직
  ...
  push/
    ...
    descriptions.py              세그먼트 별칭·Tier·설명·액션 가이드 (정적 큐레이션)
...
세그먼트 학습 노트북과 **동일 로직이어야** 하므로 한쪽을 고치면 양쪽을 맞춰야 한다.
...
- 산출물:
  ...
  - `cluster_push_stats.pkl` — 5월 실측 **세그먼트 × 푸시카테고리 × 발송시간대** 의 (노출 `n`, 오픈 `opens`)
- **k=10 선택 근거**: 세그먼트 간 오픈율 분리도가 뚜렷하고, 각 군 표본이 충분(≥ 모수 2%)하며,
...
| 스냅샷 | 피처 → 세그먼트 배정 → 저장 | `features.refresh_snapshot` → `cache/latest_cluster.csv` + `_meta.json` (회당 3~15분, 단일 실행 락) |
...
3. `artifacts/`(모델 pkl), `cache/`(스냅샷·매핑)가 repo 루트에 있어야 한다. 없으면 유저 세그먼트 화면에서 먼저 [최신화].
